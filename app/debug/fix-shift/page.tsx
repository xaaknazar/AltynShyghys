...
export default function FixShiftPage() {
  ...
  const [manualValue, setManualValue] = useState<string>(''); // Ручное значение
  ...
  const handleCheck = async () => {
    ...
    try {
      const payload: any = { date, dryRun: true };
      if (manualValue && parseFloat(manualValue) > 0) {
        payload.manualValue = parseFloat(manualValue);
      }
  ...
  const handleFix = async () => {
    ...
    try {
      const payload: any = { date, dryRun: false };
      if (manualValue && parseFloat(manualValue) > 0) {
        payload.manualValue = parseFloat(manualValue);
      }
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm mb-6">
          ...
          <div className="space-y-4">
            <div className="flex gap-4 items-end">
              ...
              <div className="flex-1">
                <label className="block text-sm text-slate-600 mb-2">
                  Ручное значение (тонн) - необязательно
                </label>
                <input
                  type="number"
                  step="0.01"
            ...
            <div className="text-sm text-slate-500">
              ...
              <p>• Если указать ручное значение, оно будет использовано вместо автоматического расчета</p>
